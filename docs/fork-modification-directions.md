...
- **核心能力**:
  ...
  - LaTeX 数学公式（双后端生产级支持：Pandoc `tex_math_dollars` + Pure Python `latex2mathml`→MathML→OMML 桥接）
- **已知限制**: 无批量转换、无 GUI、Mermaid PNG 无原生 DPI、Pandoc 长表格多余空白页、自动/手工目录冲突
...
### 3.1 Pure Python LaTeX→OMML（✅ v1.2.0 已完成）

- **当前**: v1.2.0 已实现 `latex2mathml` → MathML→OMML 桥接架构。新增 `docx_pipeline/converters/mathml2omml.py`（681 行，14 类公式结构映射），替换了原手写 LaTeX 解析器（`_parse_math_*` 系列方法）。21 个自动化测试（lxml XPath oracle），目视验证与 Pandoc 后端渲染效果一致。
- **参考来源**: MathML→OMML 映射关系参考 markdown2docx (TimeEtcher, MIT) `omml.py` 已验证映射表独立实现——两条独立推理路径收敛到同一三层架构。
- **剩余改进空间**:
  - `latex2mathml` 覆盖率有限（LaTeX 数学方言极多），不支持的公式自动回退为原文输出
  - 可考虑添加 MathBlock/InlineMath 正式 Token 类型（当前使用占位符方案，功能等价）
  - PNG 回退渲染（当 OMML 不可用时的降级路径）
- **工作量: 低**（现有架构已工作，改进为增量式）
...
4. **Pure Python 与 Pandoc 后端不是功能等价的**：Pure Python 无代码高亮。新增功能时需明确"双后端都支持？仅一边？"
...
- `docx_pipeline/converters/pure_python.py` — Pure Python 转换器（981 行）
- `docx_pipeline/converters/mathml2omml.py` — MathML→OMML 桥接层（681 行，14 类公式结构）
...
- `tests/test_pure_python_math.py` — Pure Python 数学回归测试
- `tests/test_math_bridge.py` — MathML→OMML 桥接测试（21 个，XPath oracle）
